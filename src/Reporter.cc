...
bool Reporter::PermitNetWeird(const char* name)
	...
	if ( count == 1 )
		detail::timer_mgr->Add(new NetWeirdTimer(net::network_time, name,
...
bool Reporter::PermitFlowWeird(const char* name,
	...
	if ( map.empty() )
		detail::timer_mgr->Add(new FlowWeirdTimer(net::network_time, endpoints,
		                                          weird_sampling_duration));
...
bool Reporter::PermitExpiredConnWeird(const char* name, const RecordVal& conn_id)
	...
	if ( map.empty() )
		detail::timer_mgr->Add(new ConnTupleWeirdTimer(net::network_time,
		                                               std::move(conn_tuple),
		                                               weird_sampling_duration));
...
void Reporter::DoLog(const char* prefix, EventHandlerPtr event, FILE* out,
	...
	if ( raise_event && event && via_events && ! in_error_handler )
		...
		if ( time )
			vl.emplace_back(make_intrusive<TimeVal>(
				                net::network_time ? net::network_time : util::current_time()));
	...
	if ( out )
		...
		if ( net::zeek_start_network_time != 0.0 )
			{
			char tmp[32];
			snprintf(tmp, 32, "%.6f", net::network_time);
			s += std::string(tmp) + " ";
			}
