...
double PktSrc::CurrentPacketWallClock()
	...
	if ( net::net_is_processing_suspended() )
...
double PktSrc::CheckPseudoTime()
	...
	double ct = (util::current_time(true) - first_wallclock) * net::pseudo_realtime;

	return pseudo_time <= ct ? net::zeek_start_time + pseudo_time : 0;
	}
...
void PktSrc::Process()
	...
	if ( current_packet.Layer2Valid() )
		{
		if ( net::pseudo_realtime )
			{
			current_pseudo = CheckPseudoTime();
			net::detail::net_packet_dispatch(current_pseudo, &current_packet, this);
			if ( ! first_wallclock )
				first_wallclock = util::current_time(true);
			}

		else
			net::detail::net_packet_dispatch(current_packet.time, &current_packet, this);
		}
...
bool PktSrc::ExtractNextPacketInternal()
	...
	if ( net::net_is_processing_suspended() && first_timestamp )
		return false;
...
double PktSrc::GetNextTimeout()
	...
	// then.
	if ( IsLive() || net::net_is_processing_suspended() )
		return -1;
	else if ( ! net::pseudo_realtime )
		return 0;
