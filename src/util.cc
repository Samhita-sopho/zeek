...
void init_random_seed(const char* read_file, const char* write_file,
	...
	if ( ! seeds_done )
		...
		if ( pos < zeek::detail::KeyedHash::SEED_INIT_SIZE )
			reporter->FatalError("Could not read enough random data. Wanted %d, got %lu",
...
FILE* rotate_file(const char* name, RecordVal* rotate_info)
	...
	if ( rotate_info )
		...
		rotate_info->Assign<TimeVal>(2, net::network_time);
		rotate_info->Assign<TimeVal>(3, net::network_time);
		}
...
double current_time(bool real)
	...
	if ( ! net::pseudo_realtime || real || ! iosource_mgr || ! iosource_mgr->GetPktSrc() )
		return t;
