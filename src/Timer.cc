...
void TimerMgr::Process()
	...
	if ( ! pkt_src || ! pkt_src->IsOpen() || net::reading_live || net::net_is_processing_suspended() )
	...
	net::current_dispatched += Advance(net::network_time,
	                                   max_timer_expires - net::current_dispatched);
	}
...
double PQ_TimerMgr::GetNextTimeout()
	...
	Timer* top = Top();
	if ( top )
		return std::max(0.0, top->Time() - net::network_time);
