...
SetupResult setup(int argc, char** argv,
	...
	net::pseudo_realtime = options.pseudo_realtime;
	...
	net::zeek_start_time = util::current_time(true);
	...
	for ( const auto& sf : net::detail::sig_files )
	...
	if ( dns_type != DNS_PRIME )
		net::detail::net_init(options.interface, options.pcap_file, options.pcap_output_file, options.use_watchdog);
	...
	if ( ! net::reading_live && ! net::reading_traces )
		// Set up network_time to track real-time, since
		// we don't have any other source for it.
		net::detail::net_update_time(util::current_time());
	...
	if ( zeek_script_loaded )
		...
		for ( const auto& file : net::detail::files_scanned )
			{
			if ( file.skipped )
				continue;
	...
	net::detail::have_pending_timers = ! net::reading_traces && timer_mgr->Size() > 0;
