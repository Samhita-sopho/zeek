...
: TransportLayerAnalyzer("TCP", conn)
	...
	ADD_ANALYZER_TIMER(&TCP_Analyzer::ExpireTimer,
	                   net::network_time + tcp_SYN_timeout, false,
...
void TCP_Analyzer::Done()
	...
	if ( net::terminating && connection_pending && is_active && ! BothClosed() )
...
bool TCP_Analyzer::ValidateChecksum(const struct tcphdr* tp,
	...
	{
	if ( ! net::current_pkt->l3_checksummed && ! ignore_checksums && caplen >= len &&
...
void TCP_Analyzer::DeliverPacket(int len, const u_char* data, bool is_orig,
	...
	if ( ! orig->did_close || ! resp->did_close )
		Conn()->SetLastTime(net::current_timestamp);
	...
	if ( flags.FIN() )
		...
		if ( endpoint->FIN_cnt >= tcp_storm_thresh && net::current_timestamp <
	...
	if ( flags.RST() )
		...
		if ( endpoint->RST_cnt >= tcp_storm_thresh && net::current_timestamp <
		     endpoint->last_time + tcp_storm_interarrival_thresh )
			Weird("RST_storm");
	...
	endpoint->last_time = net::current_timestamp;

	bool do_close;
	bool gen_event;
	UpdateStateMachine(net::current_timestamp, endpoint, peer, base_seq, ack_seq,
	...
	if ( DEBUG_tcp_data_sent )
		...
		DEBUG_MSG("%.6f before DataSent: len=%d caplen=%d skip=%d\n",
		          net::network_time, len, caplen, Skipping());
		}
	...
	if ( len > 0 && (caplen >= len || packet_children.size()) &&
		...
		need_contents = DeliverData(net::current_timestamp, data, len, caplen, ip,
...
void TCP_Analyzer::ConnectionClosed(TCP_Endpoint* endpoint, TCP_Endpoint* peer,
	...
	if ( DEBUG_tcp_connection_close )
		...
		DEBUG_MSG("%.6f close_complete=%d tcp_close_delay=%f\n",
		          net::network_time, close_complete, tcp_close_delay);
		}
...
bool TCPStats_Endpoint::DataSent(double /* t */, uint64_t seq, int len, int caplen,
	...
	if ( sequence_delta <= 0 )
		...
		DEBUG_MSG("%.6f rexmit %" PRIu64" + %d <= %" PRIu64" data_in_flight = %d\n",
		          net::network_time, seq, len, max_top_seq, data_in_flight);
...
void TCPStats_Analyzer::DeliverPacket(int len, const u_char* data, bool is_orig, uint64_t seq, const IP_Hdr* ip, int caplen)
	...
	if ( is_orig )
		orig_stats->DataSent(net::network_time, seq, len, caplen, data, ip, nullptr);
	else
		resp_stats->DataSent(net::network_time, seq, len, caplen, data, ip, nullptr);
	}
