...
void RPC_Interpreter::Timeout()
	...
	for ( const auto& entry : calls )
		...
		if ( c->IsValidCall() )
			...
			if ( ! RPC_BuildReply(c, BifEnum::RPC_TIMEOUT, buf, n, net::network_time, net::network_time, 0) )
				Weird("bad_RPC");
			}
		}
	}
		...
		case WAIT_FOR_MARKER:
			...
			if ( got_marker )
				...
				marker &= 0x7fffffff;
					...
					//		net::network_time, IsOrig(), marker, last_frag, msg_buf.GetExpected(), msg_buf.GetProcessed(), len);
...
RPC_Analyzer::RPC_Analyzer(const char* name, Connection* conn,
	...
	if ( Conn()->ConnTransport() == TRANSPORT_UDP )
		ADD_ANALYZER_TIMER(&RPC_Analyzer::ExpireTimer,
		                   net::network_time + rpc_timeout, true,
...
void RPC_Analyzer::DeliverPacket(int len, const u_char* data, bool orig,
	...
	if ( orig )
		{
		if ( ! interp->DeliverRPC(data, len, len, true, net::network_time, net::network_time) )
			Weird("bad_RPC");
		}
	else
		{
		if ( ! interp->DeliverRPC(data, len, len, false, net::network_time, net::network_time) )
			Weird("bad_RPC");
		}
	}
