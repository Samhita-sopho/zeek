...
void SteppingStone_Analyzer::DeliverPacket(int len, const u_char* data,
	...
	if ( is_orig )
		orig_endp->DataSent(net::network_time, seq, len, caplen, data, nullptr, nullptr);
	else
		resp_endp->DataSent(net::network_time, seq, len, caplen, data, nullptr, nullptr);
	}
...
void SteppingStone_Analyzer::DeliverStream(int len, const u_char* data,
	...
	if ( is_orig )
		{
		orig_endp->DataSent(net::network_time, orig_stream_pos, len, len,
		...
		{
		resp_endp->DataSent(net::network_time, resp_stream_pos, len, len,
		                    data, nullptr, nullptr);
		resp_stream_pos += len;
		}
	}
