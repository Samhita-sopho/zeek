...
void VXLAN_Analyzer::DeliverPacket(int len, const u_char* data, bool orig,
	...
	pkt_timeval ts;
	ts.tv_sec = (time_t) net::current_timestamp;
	ts.tv_usec = (suseconds_t) ((net::current_timestamp - (double)ts.tv_sec) * 1000000);
	...
	sessions->DoNextInnerPacket(net::network_time, &pkt, inner, estack, ec);
	}
