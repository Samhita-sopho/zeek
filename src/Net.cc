...
void net_run()
	{
	util::detail::set_processing_status("RUNNING", "net_run");
	...
	while ( iosource_mgr->Size() ||
		...
		if ( ! ready.empty() )
			...
			net_update_time(util::current_time());
			expire_timers();
			}
...
void net_finish(int drain_events)
	{
	util::detail::set_processing_status("TERMINATING", "net_finish");
...
void net_delete()
	{
	util::detail::set_processing_status("TERMINATING", "net_delete");
