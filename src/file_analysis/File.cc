...
void File::UpdateLastActivityTime()
	{
	val->Assign(last_active_idx, make_intrusive<TimeVal>(net::network_time));
	}
...
void File::ScheduleInactivityTimer() const
	{
	zeek::detail::timer_mgr->Add(new detail::FileTimer(net::network_time, id, GetTimeoutInterval()));
	}
...
void File::DeliverChunk(const u_char* data, uint64_t len, uint64_t offset)
	...
	if ( file_reassembler )
		...
		file_reassembler->NewBlock(net::network_time, offset, len, data);
		}
